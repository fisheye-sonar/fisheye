...
class YOLOARISBatchedDataset(ARISBatchedDataset):
    ...
    def _postprocess_batchwise(
        ...
        for i, labels in enumerate(frame_labels):
            ...
            # slice per-image tensors
            # img_i = imgs[i]
            # img_orig_i = imgs_original[i]  # (C, h0, w0), float32 on CPU

            output_labels.append(labels_out)

            # outputs.append((img_i, labels_out, shapes[i], img_orig_i))
        ...
        imgs = imgs / 255.0
        imgs_original = imgs_original / 255.0

        return (imgs, output_labels, shapes, imgs_original)
    ...
    def _postprocess_serial(
        ...
        for image, labels in zip(frame_images, frame_labels):
            ...
            outputs.append((img, labels_out, shapes, img_original))
        return outputs
