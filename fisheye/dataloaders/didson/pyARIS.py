# -*- coding: utf-8 -*-
"""
Modified from: https://github.com/EminentCodfish/pyARIS
...
def get_box_for_sample(beam_num, bin_num, frame, beam_data):
    ...
    sample_length = sample_period * 1e-6 * sound_speed / 2.0  # meters
...
def compute_image_bounds(
    ...
    for beam_num in [0, frame.BeamCount / 2, frame.BeamCount - 1]:
        for bin_num in [0, frame.samplesperbeam - 1]:
            bl, br, fr, fl = get_box_for_sample(
                beam_num, bin_num, frame, beam_width_data
            )
    ...
    target_center_x = (fl[0] + fr[0]) / 2.0
    target_center_y = (bl[1] + fl[1]) / 2.0
...
def compute_mapping_from_sample_to_image(
    pixel_meter_size, xdim, ydim, x_meter_start, y_meter_start, frame, beam_width_data
):

    x_meter_values = np.array(
        [x_meter_start + i * pixel_meter_size for i in range(xdim)]
    )
    y_meter_values = np.array(
        [y_meter_start - i * pixel_meter_size for i in range(ydim)]
    )
