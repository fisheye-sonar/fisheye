...
def get_xy_from_r_theta(r, theta, info):
    """Get the x and y from r and theta.
    Args:
    ...
    Returns:
        ...
        y: Y coordinate.
    """
    # get closest beam angle to theta
    ...
    # Apply warping to go from real-world to image coordinates
    warped_points = calculate_warped_points(
        [closest_beam_angle_idx, bin_num],
        info,
        info.xdim,
        info.ydim,
    )
    # Normalize coordinates back to [0, 1]
    x_norm = warped_points[0] / info.xdim
    y_norm = warped_points[1] / info.ydim
