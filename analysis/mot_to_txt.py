...
def detect_centerline_crossings(x_coords, frame_ids, center_line, x_center, y_center):
    ...
    for i in range(1, len(x_coords)):
        prev_x = x_coords.iloc[i - 1]
        curr_x = x_coords.iloc[i]

        # Check if crossing occurred
        if prev_x < center_line and curr_x >= center_line:
            # Left to right crossing
            crossings.append(
                (frame_ids.iloc[i], "down", x_center.iloc[i], y_center.iloc[i])
            )
        elif prev_x > center_line and curr_x <= center_line:
            # Right to left crossing
            crossings.append(
                (frame_ids.iloc[i], "up", x_center.iloc[i], y_center.iloc[i])
            )

    return crossings


def mot_to_txt(mot_path, info_path, start_frame, output_path=None):
...
if __name__ == "__main__":
    output_dir = "/home/mahobley/Code/fisheye/analysis/gt_files"
    input_dir = "/home/mahobley/Code/fisheye/analysis/gt_files"
    ...
    x = mot_to_txt(mot_path, info_path, start_frame, output_path)
    # print(x)
