...
4. **(Optional) Configure export output types**

    You can control which output formats are generated by the pipeline using the `export_options` field in the YAML configuration. This allows you to enable or disable specific export types depending on your needs.
    Example:
    ```
   export_options: ["summary_csv", "detailed_csv", "fc"]  # Options: "summary_csv", "detailed_csv", "fc", "xml"
   ```

    Available export types:
   - `summary_csv`: Generates a summary CSV file with aggregated metrics.
   - `detailed_csv`: Generates a detailed CSV file with per-frame/per-object data.
   - `fc`: Generates a fish count output file.
   - `xml`: Generates a MarkedFishMeasurements XML file.
   - `mot`: Generates MOT-style tracking outputs.

    Defaults to exporting `summary_csv`, `detailed_csv` and `fc`.
